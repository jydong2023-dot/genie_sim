...
| 21. Reusable USD-to-benchmark adapter | complete | Added an executable, simulator-independent CLI with assets/robot/camera validation, safe YAML+JSON generation, docs, 5 passing tests, and real host/container dry-runs. |
| 22. Add `g2_op_pick_toy` task | complete | Added the G2 pick task, target physics, checker semantics, portable material overrides, tests, and a clean no-inference Isaac preview with three validated camera images. |
| 23. Resync `g2_op_pick_toy` after scene edits | complete | Preserved user transforms, synchronized blue-block world metadata, removed stale yellow physics, updated tests, and passed a clean three-camera container preview. |
| 24. Add `blue_block` XY + yaw randomization | complete | Added collision-checked deterministic background wrappers, instances 1-10, synchronized metadata, a dedicated robust config, tests, OpenUSD validation, and three real simulator previews while preserving instance 0. |
| 25. Add `pick_multiple_toys` and rename toy prims | complete | Added three ordered, reset-separated pick episodes; renamed both referenced toy prims with preserved materials/collision; made the yellow target dynamic; passed 24 regressions and a real container preview. |
...
| Broader Dual AGX Nero test file has four stale assertions | 1 | The adapter's 5 tests pass; retain the user's newer robot pose, scripted-policy timing, and camera `x=-0.6` instead of reverting them to satisfy unrelated old expectations. |
| First `g2_op_pick_toy` preview tried the container workdir as a host workdir | 1 | No process was created and no simulator state changed; rerun through the existing `docker exec -u isaac-sim -w /workspace/source/geniesim_benchmark geniesim3 ...` path. |
| Bare container Python launchers could not import `pxr` | 1 | Use the host `geniesim` Conda OpenUSD bindings for offline inspection and the full Kit application for simulator validation. |
| First portable-override patch missed the exact phase-22 planning row | 1 | Inspect the current planning-file anchors, apply source edits separately, then update phase records with exact context. |
| `geniesim` Conda Python has no `pytest` module | 1 | Run focused tests with the repository's existing pytest interpreter and reserve the Conda environment for direct OpenUSD checks; install nothing. |
| OpenUSD one-liner referenced `pxr.UsdPhysics` without importing the submodule | 1 | Import `UsdPhysics` explicitly from `pxr` and rerun the composition assertion. |
...
| First nearby-object OpenUSD one-liner had mismatched comprehension parentheses | 1 | Simplified the read-only expression and reran it successfully; no source file was changed. |
| First randomized preview launch used the container-only `/workspace` as a host workdir | 1 | Launch through `docker exec -u isaac-sim -w /workspace/source/geniesim_benchmark geniesim3 ...`; the rejected host process never started. |
| `git -C /home/user/djy/geniesim_assets status` failed | 1 | Confirmed the separately maintained asset directory has no `.git`; validate wrapper presence/composition directly and report that these files are outside the main repository status. |
