...
| `--connect-timeout SECONDS` | Positive shared total budget for gRPC readiness, lazy client import, and the actual `RpcClient` readiness check (default 5 seconds); it does not bound the later `init_robot` RPC. |
