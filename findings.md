...
## `g2_op_pick_toy` task (2026-08-05)
- Phase 25 requirement: add `pick_multiple_toys` on the same background with ordered targets `blue_block`, `stacked_block_1`, and `yellow_push_toy`, following `pick_block_color` semantics (three instructions/problems become three reset-separated episodes when `num_episode=1`).
- The real source prim names are `yellow_plush_boy` and `blue_plush_boy`, both introduced by the referenced binary `home_toy_scene.usd`. USD `relocates` authored on the wrapper do not cross that reference arc. The verified composition is to deactivate each old prim, create the new `yellow_push_toy`/`blue_push_toy` prim with a direct object reference, and explicitly restore each external material binding.
- `yellow_plush_boy` has descendant collision but no RigidBodyAPI; the new yellow target must add dynamic rigid-body and mass schemas. `blue_block` and `stacked_block_1` already have dynamic rigid bodies.
- Phase 24 now provides deterministic XY + yaw randomization for the embedded `blue_block`. Instance `0` remains the baseline; instances `1-10` use root seed `20260805`, relative XY ranges of `[-0.03, 0.03] m`, yaw offsets of `[-30, 30] deg`, and at least `0.01 m` conservative planar clearance from automatically measured sibling AABBs.
- The generator reads the real composed background with OpenUSD, converts desired world poses into `/World/toyscene/Objects` local coordinates under the scene's `-85 deg` parent yaw, and writes asset-root-relative wrappers selected via each instance's `scenario.json.background_usd`.
- All ten wrappers compose successfully; their target world positions agree with synchronized `scene_info.json` values within `5e-7 m`. Container previews for instances `1`, `8`, and `10` loaded the corresponding wrapper paths, retained blue-block rigid-body physics, and produced nonblank head/left/right images with visibly different target positions and orientations.
- Randomization loading constraint: numeric task `scene.usda` layers are referenced at `/Workspace`, while `blue_block` lives in the background at `/World/toyscene/Objects/blue_block`. A task subscene override cannot move it; each randomized instance must select an asset-root-relative background wrapper through `scenario.json.background_usd`.
- Existing generic `object_pose.x_range/y_range` values are absolute world coordinates and its USD writer targets `/World/Objects/<id>`, so the default generic profile is not directly valid for this embedded-background target. The task-specific implementation must preserve the baseline scene and convert requested world XY/yaw offsets into the target's local transform under `/World/toyscene`.
- After the latest user-authored transforms, the composed blue-block pivot is `(0.037945286, -0.281449682, 0.751435466)`, quaternion `xyzw=(0,0,-0.675590216,0.737277329)`, and world AABB size `(0.091027534,0.091995139,0.061507840)`.
- The eval passive object, instruction text/ID, and both ADER checker paths already point to `blue_block`. Remaining inconsistencies are stale scene metadata, yellow-target physics in the wrapper, and yellow-target contract assertions.
- Phase-23 synchronization removed those inconsistencies. The latest headless preview uses the blue instruction and live blue-block checker path; all three policy cameras render correctly, and the right-hand camera frames the block directly between the gripper fingers.
- Requested source scene: `/home/user/djy/geniesim_assets/background/olalab/home_toy_scene.usd`; requested robot/task name: G2 Omnipicker / `g2_op_pick_toy`.
- The implementation must identify the actual toy prim and its physics state before selecting scoring semantics; a scene-only debug adapter would not satisfy a real pick task.
- The scene is a valid 179 MB OpenUSD stage with default prim `/World`, Z-up, meters-per-unit 1, 599 prims, existing lights, background collisions, and a tabletop object group at `/World/toyscene/Objects`.
- The strongest single target is `/World/toyscene/Objects/yellow_plush_boy`, centered near `(-0.167, 0.043, 0.795)` with bounds about `0.162 x 0.161 x 0.115 m`. Its mesh has convex-hull collision but the parent has no RigidBodyAPI, so it cannot be picked until a task-local physics override is applied.
- Other movable-ready objects (`blue_block`, `stacked_block_1`, `stacked_block_2`) already have RigidBodyAPI, but they are blocks rather than the requested plush toy.
- Existing benchmark pick scoring uses `Follow` followed by `PickUpOnGripper`; the selected prim must be discoverable under the object path convention used by these checkers.
- The checker resolver accepts absolute `/World/...` paths, so the task can score the embedded target directly without duplicating it under `/Workspace/Objects`.
- The composed wrapper successfully exposes the target with RigidBodyAPI, MassAPI (`0.1 kg`), and inherited convex-hull collision while retaining the original scene by reference.
- The source stage authored 355 texture/HDR attributes under `/home/user/djy/geniesim_assets`, which is invalid inside the Docker mount at `/geniesim_assets`; nine toy textures additionally pointed at the obsolete `background/robosnap/toy_scene` directory.
- `home_toy_scene_material_paths.usda` overrides those attributes with paths relative to `background/olalab` and redirects the nine moved textures to `olalab/toy_scene`. It is only about 75 KB and leaves `home_toy_scene.usd` unchanged.
- The final container preview resolves the task as `[g2op/if] g2op_if_g2_op_pick_toy`, initializes `Follow` and `PickUpOnGripper` against `/World/toyscene/Objects/yellow_plush_boy`, writes `evaluate_ret_01.json`, and saves head/left/right camera images with complete materials. A no-action preview correctly scores zero.

## Reusable USD-to-benchmark adapter (2026-08-04)
- The current RoboSnap adaptation is represented by two files: a runnable benchmark YAML and an eval-task JSON whose `scene_id` suffix must match the `robot_init_pose` workspace key.
- The reusable tool must not require Isaac Sim merely to generate configuration; it should perform filesystem/config validation with the standard library and PyYAML already declared by the benchmark package.
- The generated task is initially a scene-and-robot debug task. A real manipulation evaluation additionally requires the target prim/object mapping, physics properties, stage semantics, and success condition.
- `TaskGenerator.init_info()` obtains the workspace lookup key from `scene.scene_id.split("/")[-1]`; generated `scene_id`, `function_space_objects`, and `robot.robot_init_pose` therefore must share exactly the same workspace key.
- `api_core.py` prefixes `scene.scene_usd` with `system_utils.assets_path()`, so the generated path must be relative to the GenieSim assets root even when the input is an absolute host or container path.
- The package already keeps operational CLIs under `source/geniesim_benchmark/scripts/`; the adapter belongs there and can be tested through file-based module loading without importing Isaac Sim.
- Host dry-run against the real `pick_plastic_bowl.usd` resolved the expected `/background/robosnap/auto_usd/pick_plastic_bowl.usd`, three Dual AGX Nero cameras, and matching `workspace_00` keys without creating output files.
- The container's system Python includes PyYAML 6.0.3, and the executable script runs directly as the `isaac-sim` user; no Isaac Sim startup or package installation is needed to generate configs.
...
- `pick_multiple_toys` follows the existing `pick_block_color` episode model: with three instructions/problems and `benchmark.num_episode=1`, the benchmark sends blue block, stacked block, and yellow toy instructions in that order across three environment resets.
- The source scene names are actually `yellow_plush_boy` and `blue_plush_boy`. Because those prims arrive through a reference arc, the editable wrapper cannot relocate them directly; it deactivates the old prims and directly references them as `yellow_push_toy` and `blue_push_toy`, with explicit material bindings restored.
- OpenUSD contract checks confirm both renamed toys retain collision and materials. `yellow_push_toy` additionally has enabled dynamic rigid-body physics and a 0.1 kg mass so its `PickUpOnGripper` checker is physically achievable.
- The existing `geniesim3` container preview exited 0, loaded G2 and the edited background, detected all three task targets as rigid bodies, initialized the first right-gripper checker, and produced a non-empty head-camera image with the expected instruction overlay.
